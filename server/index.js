...
app.use('/admin', express.static(path.join(__dirname, 'admin'))) // 将uploads文件夹下的内容托管为静态资源
app.use('/', express.static(path.join(__dirname, 'web'))) // 将uploads文件夹下的内容托管为静态资源

require('./plugins/db')(app) // 引入db模块，并向其中传入express对象
require('./routes/admin')(app) // 引入路由模块，并向其中传入express模块
require('./routes/web')(app) // 引入web端的路由
